...
export default class OrcamentoScreen extends React.Component {
  ...
  // TODO
  componentDidMount() {
    const orcID = this.props.navigation.getParam('ID', undefined);
    //Alert.alert(orcID)

    fetch(global.serverUrl + '/sales/document/' + orcID, global.getOptions)
    .then(response => response.json())
    .then(orcamento => {
      console.log(orcamento)
      console.log(orcamento.header)
      this.setState({
        orcamento
      })
    })
   ...
   _onPressButton() {
    Alert.alert('Orçamento #0012023 criado!')
    ...
    <View style={styles.container}> 
       ...
       <View style={styles.paper}>
        ...
        <View style={styles.rowA}> 
          ...
          <View style={styles.childrenA}>
              <ImageBackground source={require('../assets/images/orc.png')} style={styles.bg}></ImageBackground>
          </View>
          ...
          <View style={styles.childrenB}>   
            <View style={styles.text}>
              ...
              <View style={{marginBottom: 6}}>
                <Text style={{color: '#85adad'}}> {this.state.orcamento.header.Nome} </Text>
              </View>
              

              <View style={{marginBottom: 0, marginTop: 8}}>
                <Text> Inicio:  {this.props.from} </Text>
                <Text style={{marginTop:6, color: '#85adad'}}> {this.state.orcamento.header.DataCriacao} </Text>
              </View>
              <View style={{marginTop:6}}>
                <Text> Fim: {this.props.to}</Text>
                <Text style={{marginTop:6, color: '#85adad'}}> {this.state.orcamento.header.DataCarga} </Text>
              </View>     
        ...
        <View style={styles.textTitle} style={{marginTop:0}}>
          <Text> Codigo: </Text>
          <Text style={{marginTop:6, color: '#85adad'}}> #2018{this.state.orcamento.header.NumDoc} </Text>
        </View>
        ...
        <View style={styles.rawTubos} style={{flexDirection:'column', marginTop: 12, marginLeft:5}}  >
           { this.state.orcamento.rows.map((item, key)=>(
      ...
      {/* Preco Total */}
        <View style={styles.textTitle} style={{marginTop: 8}}>
          ...
          <Text style={{marginTop:6, color: '#85adad'}}> {this.state.orcamento.header.TotalDocumento}€ </Text>
        </View>
